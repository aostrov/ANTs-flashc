...
::InitializeRegistrationAtEachLevel( const SizeValueType level )
{
  ...
  if( level == 0 )
    ...
    // debug
    ITKFileIO::SaveImage(*v0Spatial, "v0_spatial_before_resample.nii.gz");
    // end: debug
    Opers::Resample(*v0SpatialNew, *v0Spatial);
    // debug
    ITKFileIO::SaveImage(*v0SpatialNew, "v0_spatial_after_resample.nii.gz");
    // end: debug
    this->m_fftoper = new FftOper(this->m_LaplacianWeight,
                                  this->m_IdentityWeight,
                                  this->m_OperatorOrder,
    ...
    //debug
    this->m_fftoper->fourier2spatial(*v0Spatial, *(this->m_v0));
    ITKFileIO::SaveImage(*v0Spatial, "v0_spatial_after_fft.nii.gz");
    //end debug
